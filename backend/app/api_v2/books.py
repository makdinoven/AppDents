...
@router.get(
    "/filters/authors/search",
    response_model=FilterSearchResponse,
    summary="Поиск авторов для фильтра",
    description="""
    Поиск авторов с автокомплитом для использования в фильтрах.
    
    Возвращает список авторов с количеством книжных лендингов для каждого.
    Учитывает текущие фильтры для актуальности counts.
    
    ### Параметры:
    - **q** - поисковый запрос (минимум 1 символ)
    - **limit** - максимальное количество результатов (по умолчанию 20)
    - Все параметры фильтрации из основного эндпоинта (language, tags, formats, и т.д.)
    
    ### Примеры:
    ```
    GET /filters/authors/search?q=smith&limit=10
    GET /filters/authors/search?q=john&language=EN&limit=20
    ```
    """,
    tags=["public"]
)
def search_authors(
    ...
    # Фильтры для учета контекста
    language: Optional[str] = Query(None, regex="^(EN|RU|ES|IT|AR|PT)$"),
    tags: Optional[List[str]] = Query(None),
    formats: Optional[List[str]] = Query(None),
    publisher_ids: Optional[List[int]] = Query(None),
    ...
    pages_from: Optional[int] = Query(None, ge=0),
    pages_to: Optional[int] = Query(None, ge=0),
    db: Session = Depends(get_db),
):
    """Поиск авторов с учетом текущих фильтров."""
...
@router.get(
    "/filters/publishers/search",
    response_model=FilterSearchResponse,
    summary="Поиск издателей для фильтра",
    description="""
    Поиск издателей с автокомплитом для использования в фильтрах.
    
    Возвращает список издателей с количеством книжных лендингов для каждого.
    Учитывает текущие фильтры для актуальности counts.
    """,
    tags=["public"]
)
def search_publishers(
    ...
    """Поиск издателей с учетом текущих фильтров."""
...
@router.get(
    "/filters/tags/search",
    response_model=FilterSearchResponse,
    summary="Поиск тегов для фильтра",
    description="""
    Поиск тегов с автокомплитом для использования в фильтрах.
    
    Возвращает список тегов с количеством книжных лендингов для каждого.
    Учитывает текущие фильтры для актуальности counts.
    """,
    tags=["public"]
)
def search_tags(
    ...
    """Поиск тегов с учетом текущих фильтров."""
