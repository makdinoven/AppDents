...
def add_landing(landing: LandingCreate, db: Session = Depends(get_db)):
    try:
...
def update_landing_endpoint(landing_id: int, landing: LandingUpdate, db: Session = Depends(get_db)):
    try:
...
def delete_landing_endpoint(landing_id: int, db: Session = Depends(get_db)):
    try:
