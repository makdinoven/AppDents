...
class SuppressionType(str, PyEnum):
    ...
    # ВАЖНО: фактические значения в БД уже в нижнем регистре (например 'invalid'),
    # поэтому .value оставляем lowercase. А чтобы SQLAlchemy не использовал ИМЕНА enum
    # (HARD_BOUNCE/...), ниже в колонке Enum(...) задаём values_callable.
